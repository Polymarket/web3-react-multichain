...
function reducer(
    ...
    switch (type) {
        ...
        default: {
            throw new Error(
                "This should never be thrown but we need to quiet eslint",
            );
        }
    }
}
...
export function useWeb3ReactManager(): Web3ReactManagerReturn {
    ...
    const getProvider = useCallback(
        async (_chainId: number) => {
            if (!connector) {
                throw new Error(
                    "Cannot call `getProvider` before calling `activate`",
                );
            }
            ...
            if (
                ...
                throw new Error(
                    `Unsupported chain id: ${chainId}. Supported chain ids are: ${connector.supportedChainIds}.`,
                );
            }
        ...
        async (
            ...
            try {
                ...
                if (error instanceof StaleConnectorError) {
                    ...
                    warning(
                        false,
                        `Suppressed stale connector activation ${connector}`,
                    );
                } else if (throwErrors) {
                    if (activated) {
                        await connector.deactivate();
